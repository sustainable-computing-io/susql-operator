...
# ibm.com/susql-operator-bundle:$VERSION and ibm.com/susql-operator-catalog:$VERSION.
IMAGE_TAG_BASE ?= quay.io/sustainable_computing_io/susql_operator
...
.PHONY: all
all: build operator-build bundle bundle-build
...
deploy: manifests kustomize ## Deploy controller to the K8s cluster specified in ~/.kube/config.
	cd config/manager && $(KUSTOMIZE) edit set image controller=${IMG}
	$(KUSTOMIZE) build config/default |sed -e "s|<OPERATOR_IMG>|$(OPERATOR_IMG)|g" | $(KUBECTL) apply -f -
